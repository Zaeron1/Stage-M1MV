# FUSE2_SAM_min_show.py — détection fumerolle (ROI + seed + masque exclusion) + SAM + debug images + show=True
# Image = matrices. On calcule: luminance Y, anomalie |ΔY|, texture σ locale, segmentation core/edge,
# composantes, choix meilleur label, avec SAM comme générateur de candidats.
...
# ---------- SAM CONFIG ----------
SAM_CKPT = os.path.join(BASE, "sam_b.pt")
...
def sam_labels_from_roi(rgb_roi, allowed_roi):
    """
    Retourne (lab_sam, n_sam) où lab_sam est une image de labels (0=fond),
    n_sam = nombre de labels.
    """
    masks = mask_generator.generate(rgb_roi)  # RGB uint8 (H,W,3)
    H, W = rgb_roi.shape[:2]
    lab_sam = np.zeros((H, W), dtype=np.int32)
    cur_label = 1
    for m in masks:
        ...
        if seg.sum() < S_MIN:
            continue
        lab_sam[seg] = cur_label
        cur_label += 1
    return lab_sam, (cur_label - 1)
...
ROI = clamp(ROI, Wf, Hf)
SEED = clamp(SEED, Wf, Hf) if SEED is not None else None
if ROI is None:
...
# 1) Labels SAM dans la ROI
lab_sam, n_sam = sam_labels_from_roi(rgb, allowed)

if n_sam > 0:
    lab = lab_sam
    n = n_sam + 1  # 0 = fond
    flat = lab.reshape(-1)
    area = np.bincount(flat, minlength=n).astype(np.int32)
    stats = np.zeros((n, 5), dtype=np.int32)
    stats[:, cv2.CC_STAT_AREA] = area
else:
    # Fallback : segmentation edge classique
    n, lab, stats, _ = cv2.connectedComponentsWithStats((edge.astype(np.uint8)*255), 8)
    if n <= 1:
        raise RuntimeError("Aucune composante")

# 2) Choix des labels touchant seed ou core
...
# 3) Score physique sur chaque label
