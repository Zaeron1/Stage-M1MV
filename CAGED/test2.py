import numpy as np
from PIL import Image
import matplotlib.pyplot as plt

from skimage.morphology import (
    opening, closing, disk,
    remove_small_objects, dilation
)
from skimage.filters import rank
from skimage.util import img_as_ubyte
from skimage.segmentation import flood
from tqdm import tqdm


# ============================================================
# UTILITAIRES
# ============================================================

def rgb_to_luminance(img_rgb):
    img = img_rgb.astype(np.float32)
    R, G, B = img[..., 0], img[..., 1], img[..., 2]
    return 0.2126 * R + 0.7152 * G + 0.0722 * B


def local_std(image, win=9):
    image_n = (image - image.min()) / (image.max() - image.min() + 1e-6)
    image_u8 = img_as_ubyte(image_n)
    mean = rank.mean(image_u8, disk(win // 2))
    mean_sq = rank.mean(image_u8 ** 2, disk(win // 2))
    return np.sqrt(mean_sq - mean ** 2)


def dominance_ratio(arr):
    p50 = np.percentile(arr, 50)
    p90 = np.percentile(arr, 90)
    p99 = np.percentile(arr, 99)
    return (p99 - p90) / (p90 - p50 + 1e-6)


# ============================================================
# DÉTECTION PANACHE — STRICTEMENT RELATIVE AU ROI
# ============================================================

def detect_panache_visible(
    image_rgb,
    roi,
    S_min=60,
    k_core=2.2,
    k_edge=1.1,
    win_tex=9,
    show=True
):
    """
    Détection de panache visible basée uniquement sur
    des ruptures statistiques DOMINANTES internes au ROI.
    """

    # --------------------------------------------------------
    # 1. EXTRACTION ROI
    # --------------------------------------------------------
    x1, y1, x2, y2 = roi
    roi_rgb = image_rgb[y1:y2, x1:x2]

    # --------------------------------------------------------
    # 2. LUMINANCE NORMALISÉE
    # --------------------------------------------------------
    Y = rgb_to_luminance(roi_rgb)
    Yn = (Y - np.median(Y)) / (np.std(Y) + 1e-6)

    # --------------------------------------------------------
    # 3. ANOMALIE & TEXTURE
    # --------------------------------------------------------
    Y_bg = np.percentile(Yn, 30)
    deltaY = np.abs(Yn - Y_bg)
    sigma_local = local_std(Yn, win=win_tex)

    # --------------------------------------------------------
    # 4. TESTS DE DOMINANCE (GARDE-FOU ABSOLU)
    # --------------------------------------------------------
    dom_Y = dominance_ratio(deltaY)
    dom_sigma = dominance_ratio(sigma_local)

    if dom_Y < 1.2 and dom_sigma < 1.2:
        # ROI homogène → PAS DE PANACHE
        empty_mask = np.zeros_like(deltaY, dtype=bool)
        metrics = {
            "panache": False,
            "reason": "No dominant internal contrast",
            "dominance_luminance": dom_Y,
            "dominance_texture": dom_sigma
        }

        if show:
            plt.figure(figsize=(6, 4))
            plt.imshow(roi_rgb)
            plt.title("ROI homogène — PAS DE PANACHE")
            plt.axis("off")
            plt.show()

        return False, empty_mask, metrics

    # --------------------------------------------------------
    # 5. NOYAU STRICT (LUMINANCE DOMINANTE)
    # --------------------------------------------------------
    sigma_Y = np.std(Yn)
    thr_core = k_core * sigma_Y

    mask_core = deltaY > thr_core
    mask_core = opening(mask_core, disk(2))
    mask_core = remove_small_objects(mask_core, min_size=S_min)

    # --------------------------------------------------------
    # 6. EXTENSION CONDITIONNELLE (ΔY OU σ)
    # --------------------------------------------------------
    thr_edge = k_edge * sigma_Y
    sigma_thr = np.percentile(sigma_local, 70)

    mask_ext = np.zeros_like(mask_core)
    seeds = np.argwhere(mask_core)

    for y, x in tqdm(seeds, desc="Extension du panache", unit="seed"):
        flooded = flood(deltaY, (y, x), tolerance=thr_edge)
        flooded &= (deltaY > thr_edge) | (sigma_local > sigma_thr)
        mask_ext |= flooded

    # --------------------------------------------------------
    # 7. DILATATION AVANT NETTOYAGE
    # --------------------------------------------------------
    mask_ext = dilation(mask_ext, disk(2))

    # --------------------------------------------------------
    # 8. NETTOYAGE FINAL
    # --------------------------------------------------------
    mask_ext = closing(mask_ext, disk(3))
    mask_ext = remove_small_objects(mask_ext, min_size=S_min)

    # --------------------------------------------------------
    # 9. MÉTRIQUES
    # --------------------------------------------------------
    area = int(mask_ext.sum())

    panache = area >= S_min

    metrics = {
        "panache": panache,
        "area_px": area,
        "dominance_luminance": dom_Y,
        "dominance_texture": dom_sigma,
        "thr_core": thr_core,
        "thr_edge": thr_edge
    }

    # --------------------------------------------------------
    # 10. DEBUG VISUEL
    # --------------------------------------------------------
    if show:
        fig, ax = plt.subplots(2, 4, figsize=(18, 8))

        ax[0, 0].imshow(image_rgb)
        ax[0, 0].add_patch(
            plt.Rectangle((x1, y1), x2-x1, y2-y1,
                          edgecolor="red", facecolor="none", lw=2)
        )
        ax[0, 0].set_title("Image + ROI")
        ax[0, 0].axis("off")

        ax[0, 1].imshow(deltaY, cmap="inferno")
        ax[0, 1].set_title("|ΔY|")
        ax[0, 1].axis("off")

        ax[0, 2].imshow(sigma_local, cmap="viridis")
        ax[0, 2].set_title("σ locale")
        ax[0, 2].axis("off")

        ax[0, 3].imshow(mask_core, cmap="gray")
        ax[0, 3].set_title("Noyau strict")
        ax[0, 3].axis("off")

        ax[1, 0].imshow(mask_ext, cmap="gray")
        ax[1, 0].set_title("Masque final")
        ax[1, 0].axis("off")

        ax[1, 1].imshow(Y, cmap="gray")
        ax[1, 1].set_title("Luminance Y")
        ax[1, 1].axis("off")

        ax[1, 2].imshow(Y, cmap="gray")
        ax[1, 2].imshow(mask_ext, cmap="Reds", alpha=0.5)
        ax[1, 2].set_title(
            f"Résultat : {'PANACHE' if panache else 'PAS DE PANACHE'}"
        )
        ax[1, 2].axis("off")

        ax[1, 3].axis("off")

        plt.tight_layout()
        plt.show()

    return panache, mask_ext, metrics


# ============================================================
# EXEMPLE
# ============================================================

if __name__ == "__main__":
    IMAGE_PATH = "CAGED/images/image6.png"
    ROI = (500, 600, 1300, 1800)

    img = np.array(Image.open(IMAGE_PATH).convert("RGB"))

    panache, mask, metrics = detect_panache_visible(
        img,
        ROI,
        show=True
    )

    print("Panache détecté :", panache)
    for k, v in metrics.items():
        print(f"{k} : {v}")

